...
func (p Permission) validate() error {
	...
	if p.Name == "" || !nameShapeRe.MatchString(p.Name) {
	...
	if prefix != expectedPrefix {
		ok := len(prefix) > len(expectedPrefix) && prefix[:len(expectedPrefix)] == expectedPrefix && prefix[len(expectedPrefix)] == '.'
		if !ok {
