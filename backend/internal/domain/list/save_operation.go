...
func (o SaveOperation) Validate() error {
	...
	if o.Status == SaveOperationStatusCompensated &&
		o.CompensatedAt == nil {
		return fmt.Errorf(
			"%w: compensatedAt is required for compensated status",
			ErrInvalidSaveOperation,
		)
	}
	...
	if o.Status != SaveOperationStatusCompensated &&
		o.CompensatedAt != nil {
		return fmt.Errorf(
			"%w: compensatedAt is allowed only for compensated status",
			ErrInvalidSaveOperation,
		)
	}
...
func validateSaveOperationPayload(
	listID string,
	payload SaveOperationPayload,
) error {
	...
	if targetListID != "" && targetListID != listID {
		return fmt.Errorf(
			"%w: target list id %q does not match operation list id %q",
			ErrInvalidSaveOperation,
			targetListID,
			listID,
		)
	...
	for index, image := range payload.NewImages {
		...
		if imageID == "" {
			return fmt.Errorf(
				"%w: newImages[%d].imageId is required",
				ErrInvalidSaveOperation,
				index,
			)
		}
		...
		if strings.Contains(imageID, "/") ||
			...
			return fmt.Errorf(
				"%w: newImages[%d].imageId is invalid",
				ErrInvalidSaveOperation,
				index,
			)
		}
		...
		if image.DisplayOrder < 0 {
			return fmt.Errorf(
				"%w: newImages[%d].displayOrder must be >= 0",
				ErrInvalidSaveOperation,
				index,
			)
		}
	...
	for index, rawImageID := range payload.DeleteImageIDs {
		...
		if _, alsoCreated := imageIDs[imageID]; alsoCreated {
			return fmt.Errorf(
				"%w: image %q cannot be created and deleted in the same operation",
				ErrInvalidSaveOperation,
				imageID,
			)
		}
...
func cloneSaveOperationPayload(
	source SaveOperationPayload,
) SaveOperationPayload {
	target := source

	if source.PreviousList != nil {
		previousList := *source.PreviousList
		target.PreviousList = &previousList
	}

	target.NewImages = append(
		[]SaveOperationImage(nil),
		source.NewImages...,
	)

	target.DeleteImageIDs = append(
		[]string(nil),
		source.DeleteImageIDs...,
	)

	target.PreviousImages = append(
		[]ListImage(nil),
		source.PreviousImages...,
	)
