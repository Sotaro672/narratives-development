...
// CampaignsTableDDL defines the SQL for the campaigns table.
const CampaignsTableDDL = `
...
BEGIN
  IF EXISTS (
    ...
    BEGIN
      ALTER TABLE campaigns
        ADD CONSTRAINT fk_campaign_assignee
...
// （任意）DDL: 参照整合性のための外部キーを追加（存在すればスキップ）
const CampaignsFKDDL = `
DO $$
BEGIN
  -- brand_id -> brands(id)
  ...
  -- performance_id -> campaign_performances(id) (nullable)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='campaign_performances') THEN
    BEGIN
      ALTER TABLE campaigns
        ADD CONSTRAINT fk_campaign_performance
        FOREIGN KEY (performance_id) REFERENCES campaign_performances(id) ON DELETE SET NULL;
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
  END IF;

  -- image_id -> campaign_images(id) (nullable)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='campaign_images') THEN
    BEGIN
      ALTER TABLE campaigns
        ADD CONSTRAINT fk_campaign_image
