...
//     modelResolver が DI されていれば modelId(variationID)->size/color/rgb/modelNumber を埋める。なければ空で返す（500にしない）。
//
import (
	"context"
	"errors"
...
// ✅ modelId(variationID) -> (size/color/rgb/modelNumber)（best-effort）
		...
		for _, ord := range pr.Items {
			...
			for _, it := range ord.Items {
				...
				if it.ModelID != "" {
					...
					if mr.RGB != nil {
						// NOTE: mr.RGB が *int の想定。もし *int64 なら strconv.FormatInt に変更する。
