...
func NewListSaveOperationUsecase(p NewListSaveOperationUsecaseParams) *ListSaveOperationUsecase {
	...
	return &ListSaveOperationUsecase{
		...
		cartItemCleanup:  p.CartItemCleanup,
		now:              now,
...
func (uc *ListSaveOperationUsecase) executeLoaded(ctx context.Context, operation listdom.SaveOperation) (listdom.SaveOperation, error) {
	...
	case listdom.SaveOperationStatusCompleted,
		...
		listdom.SaveOperationStatusFailedRetryable:
	...
	for {
		...
		case listdom.SaveOperationStatusSettingPrimary:
			...
			if err := uc.cleanupCartForSuspendedList(ctx, operation); err != nil {
				return uc.failExecution(ctx, operation, err)
			}
...
func (uc *ListSaveOperationUsecase) applyPrimaryImage(ctx context.Context, operation listdom.SaveOperation) error {
	...
	if operation.Payload.TargetList.UpdatedBy != nil {
		updatedBy := *operation.Payload.TargetList.UpdatedBy
		if updatedBy != "" {
...
func (uc *ListSaveOperationUsecase) cleanupCartForSuspendedList(ctx context.Context, operation listdom.SaveOperation) error {
	if operation.Type != listdom.SaveOperationTypeUpdate {
		return nil
	}
	if operation.Payload.PreviousList == nil {
		return nil
	}
	if operation.Payload.PreviousList.Status != listdom.StatusListing ||
		operation.Payload.TargetList.Status != listdom.StatusSuspended {
		return nil
	}
	if uc.cartItemCleanup == nil {
		return errors.New("cart item cleanup is nil")
	}

	return uc.cartItemCleanup.RemoveItemsByListID(ctx, operation.ListID)
}
...
func (uc *ListSaveOperationUsecase) persistMutation(
	ctx context.Context,
	operation listdom.SaveOperation,
	mutate func(*listdom.SaveOperation) error,
...
func (uc *ListSaveOperationUsecase) preparePayloadState(
	ctx context.Context,
	input StartListSaveOperationInput,
	listID string,
	now time.Time,
) (listdom.List, *listdom.List, []listdom.ListImage, string, error) {
...
func sameCreatedListIdentity(existing listdom.List, target listdom.List) bool {
	return existing.ID == target.ID &&
		existing.InventoryID == target.InventoryID &&
		existing.CreatedBy == target.CreatedBy
}
...
func defaultListSaveOperationRetryableError(err error) bool {
	return errors.Is(err, context.DeadlineExceeded) ||
		...
		errors.Is(err, listdom.ErrSaveOperationConflict)
}
