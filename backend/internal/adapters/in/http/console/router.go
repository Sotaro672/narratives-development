...
type RouterDeps struct {
	...
	// endpoint:
	//   POST /internal/invitations/deliveries/process
	//
	// Cloud Scheduler等から呼ばれる期限到来delivery投入用です。
	// endpoint:
	...
	//
	// Cloud Scheduler等から呼ばれるSettlement再投入用です。
	// endpoint:
...
func NewRouter(deps RouterDeps) http.Handler {
	...
	if deps.ReportDecisionNotifications != nil {
		h := withAuth(deps.ReportDecisionNotifications)
		mux.Handle("/report-decision-notifications", h)
		mux.Handle("/report-decision-notifications/", h)
	}
	...
	if deps.ListSaveOperations != nil {
		h := withAuth(deps.ListSaveOperations)
		mux.Handle("/lists/save-operations", h)
		mux.Handle("/lists/save-operations/", h)
	}
	...
	if deps.InternalSettlementDispatch != nil {
		h := withPublic(deps.InternalSettlementDispatch)
		mux.Handle("/internal/settlements/dispatch-due", h)
	}
