// backend\internal\adapters\out\firestore\inventory_repository_pg.go
package firestore
...
// InventoryRepositoryPG implements usecase.InventoryRepo with PostgreSQL.
...
// Extras (not required by the interface, but useful for listing/searching)
...
// ★★★ ここをインターフェースに合わせて修正 ★★★
func (r *InventoryRepositoryPG) Save(ctx context.Context, inv invdom.Inventory) (invdom.Inventory, error) {
	modelsJSON, err := json.Marshal(inv.Models)
	if err != nil {
		return invdom.Inventory{}, err
	}
...
func buildInventoryWhere(f invdom.Filter) ([]string, []any) {
	...
	if sq := strings.TrimSpace(f.SearchQuery); sq != "" {
		where = append(where, fmt.Sprintf(
			"(id ILIKE $%d OR location ILIKE $%d OR created_by ILIKE $%d OR updated_by ILIKE $%d)",
