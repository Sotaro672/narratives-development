...
// NullableTrim returns nil for nil/blank pointers, otherwise the string.
...
// ContainsString は、v が slice 内に含まれているかを判定します。
...
// IntersectsStrings は、2つのスライスに共通要素(空文字除外)があるかどうかを返します。
...
// HasAllStrings は、need の各要素(空文字除外)がすべて have に含まれているかを判定します。
