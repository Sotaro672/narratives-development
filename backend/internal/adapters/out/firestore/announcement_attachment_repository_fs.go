...
//
// This repository manages attachment metadata only.
//
// Firebase Storage objects are managed separately by:
// backend/internal/adapters/out/firebase/announcement_attachment_storage.go
//
// Aggregate-wide deletion of:
//
// announcements/{announcementId}/attachments/*
//
// is handled by AnnouncementRepositoryFS.Delete.
type AnnouncementAttachmentRepositoryFS struct {
	Client *firestore.Client
}

func NewAnnouncementAttachmentRepositoryFS(
	client *firestore.Client,
) *AnnouncementAttachmentRepositoryFS {
	return &AnnouncementAttachmentRepositoryFS{
		Client: client,
	}
}
...
// ListByAnnouncementID retrieves all attachment metadata documents
...
// Create inserts one attachment metadata document.
//
// This method does not upload a Firebase Storage object.
func (r *AnnouncementAttachmentRepositoryFS) Create(
	ctx context.Context,
	f announcement.AttachmentFile,
) (announcement.AttachmentFile, error) {
	if r.Client == nil {
		return announcement.AttachmentFile{},
			errors.New(
				"firestore client is nil",
			)
	}
	...
	if f.ID == "" {
		f.ID = announcement.MakeAttachmentID(
			f.AnnouncementID,
			f.FileName,
		)
	}
...
// Update applies a patch to one attachment metadata document.
//
// This method does not update or move the corresponding
...
func (r *AnnouncementAttachmentRepositoryFS) Update(
	...
	if fileName == "" {
		return announcement.AttachmentFile{},
			announcement.ErrInvalidFileName
	}
...
// Delete physically deletes one attachment metadata document.
//
// This method does not delete the corresponding Firebase Storage object.
//
// Aggregate-wide deletion is handled by AnnouncementRepositoryFS.Delete,
// after AnnouncementAttachmentStorage.DeleteAll has removed the
