...
// 現在の読者が対象 News を既読かどうか判定するためだけに利用する。
...
func (r *NewsReadRepositoryFS) CreateIfAbsent(
	...
	existing, err := decodeNewsReadForRecipient(
		snapshot,
...
func (r *NewsReadRepositoryFS) Get(
	...
	read, err := decodeNewsReadForRecipient(snapshot, recipientType, recipientID)
	if err != nil {
		return newsdom.NewsRead{}, err
	}
	if read.NewsID != newsID {
		return newsdom.NewsRead{}, newsdom.ErrInvalidReadID
	}

	return read, nil
}

func (r *NewsReadRepositoryFS) GetMany(
	ctx context.Context,
	newsIDs []newsdom.NewsID,
	recipientType newsdom.RecipientType,
	recipientID string,
) ([]newsdom.NewsRead, error) {
	if r == nil || r.client == nil {
		return nil, fmt.Errorf("news read repository is not configured")
	}
	if err := recipientType.Validate(); err != nil {
		return nil, err
	}
	if recipientID == "" {
		return nil, newsdom.ErrInvalidRecipientID
	}
	if len(newsIDs) == 0 {
		return []newsdom.NewsRead{}, nil
	}

	refs := make([]*firestore.DocumentRef, 0, len(newsIDs))
	seenNewsIDs := make(map[newsdom.NewsID]struct{}, len(newsIDs))

	for _, newsID := range newsIDs {
		if err := newsID.Validate(); err != nil {
			return nil, err
		}
		if _, exists := seenNewsIDs[newsID]; exists {
			continue
		}
		...
		seenNewsIDs[newsID] = struct{}{}
		refs = append(refs, r.doc(readID))
	}
	...
	snapshots, err := r.client.GetAll(ctx, refs)
	if err != nil {
		return nil, err
	}

	reads := make([]newsdom.NewsRead, 0, len(snapshots))
...
func decodeNews(snapshot *firestore.DocumentSnapshot) (newsdom.News, error) {
	...
	if document.Image != nil {
		image = &newsdom.NewsImage{
			FileURL:    document.Image.FileURL,
			ObjectPath: document.Image.ObjectPath,
			FileName:   document.Image.FileName,
			MimeType:   document.Image.MimeType,
			FileSize:   document.Image.FileSize,
...
func decodeNewsReadForRecipient(
