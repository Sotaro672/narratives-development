...
func (r *MemberRepositoryFS) GetByID(ctx context.Context, id string) (memdom.Record, error) {
	...
	if err != nil {
		...
		return memdom.Record{}, fmt.Errorf("get member %q: %w", id, err)
	}

	m, err := readMemberSnapshot(doc)
	if err != nil {
		return memdom.Record{}, fmt.Errorf("decode member %q: %w", id, err)
	}
...
func (r *MemberRepositoryFS) GetByUID(ctx context.Context, uid string) (memdom.Record, error) {
	...
	uidDoc, err := r.memberUIDsCol().Doc(uid).Get(ctx)
	if err != nil {
		if status.Code(err) == codes.NotFound {
			return memdom.Record{}, memdom.ErrNotFound
		}
		return memdom.Record{}, fmt.Errorf("get member UID mapping %q: %w", uid, err)
	}

	mapping, err := readMemberUIDSnapshot(uidDoc)
	if err != nil {
		return memdom.Record{}, err
	}

	rec, err := r.GetByID(ctx, mapping.MemberID)
	if err != nil {
		return memdom.Record{}, err
	}
	if rec.Member.UID != uid {
...
func (r *MemberRepositoryFS) ListByCompanyID(ctx context.Context, companyID string, _ memdom.Filter, p memdom.Page) (memdom.RecordPageResult, error) {
	if r.Client == nil {
...
func (r *MemberRepositoryFS) Create(ctx context.Context, m memdom.Member) (memdom.Record, error) {
	...
	if m.UID == "" {
		if _, err := memberRef.Create(ctx, m); err != nil {
			...
			return memdom.Record{}, fmt.Errorf("create member %q: %w", memberRef.ID, err)
		}
	...
	err := r.Client.RunTransaction(ctx, func(ctx context.Context, tx *firestore.Transaction) error {
		_, exists, err := getMemberUIDMappingInTransaction(tx, uidRef)
		if err != nil {
			return err
		}
		if exists {
		...
		if err := tx.Create(memberRef, m); err != nil {
			return fmt.Errorf("create member %q in transaction: %w", memberRef.ID, err)
		}
		...
		if err := tx.Create(uidRef, memberUIDDocument{
			...
			return fmt.Errorf("create member UID mapping %q: %w", m.UID, err)
		}

		return nil
	})
	if err != nil {
		if errors.Is(err, memdom.ErrConflict) || status.Code(err) == codes.AlreadyExists {
			return memdom.Record{}, memdom.ErrConflict
		}
		return memdom.Record{}, fmt.Errorf("create member transaction: %w", err)
	}
...
func (r *MemberRepositoryFS) Update(ctx context.Context, id string, patch memdom.MemberPatch) (memdom.Record, error) {
	...
	err := r.Client.RunTransaction(ctx, func(ctx context.Context, tx *firestore.Transaction) error {
		...
		if err != nil {
			...
			return fmt.Errorf("get member %q in transaction: %w", id, err)
		}

		current, err := readMemberSnapshot(memberDoc)
		if err != nil {
			return fmt.Errorf("decode member %q in transaction: %w", id, err)
		}

		now := time.Now().UTC()
		updated, err := applyMemberPatch(current, patch, now)
		if err != nil {
			return err
		}
		...
		if newUID != "" {
			...
			if newUIDExists && newUIDMapping.MemberID != id {
				return memdom.ErrConflict
			}
		}
		...
		if oldUID != "" && oldUID != newUID {
			...
			deleteOldUID = oldUIDExists && oldUIDMapping.MemberID == id
		}

		if err := tx.Set(memberRef, updated); err != nil {
			return fmt.Errorf("update member %q in transaction: %w", id, err)
		}
	...
	if err != nil {
		...
		case errors.Is(err, memdom.ErrNotFound):
			return memdom.Record{}, memdom.ErrNotFound
		case errors.Is(err, memdom.ErrConflict), status.Code(err) == codes.AlreadyExists:
			return memdom.Record{}, memdom.ErrConflict
		default:
			return memdom.Record{}, fmt.Errorf("update member transaction: %w", err)
		}
	}
...
func (r *MemberRepositoryFS) Delete(ctx context.Context, id string) error {
	...
	err := r.Client.RunTransaction(ctx, func(ctx context.Context, tx *firestore.Transaction) error {
		...
		if err != nil {
			...
			return fmt.Errorf("get member %q before delete: %w", id, err)
		}

		member, err := readMemberSnapshot(memberDoc)
		if err != nil {
			return fmt.Errorf("decode member %q before delete: %w", id, err)
		}
		...
		if uid != "" {
			...
			mapping, exists, err := getMemberUIDMappingInTransaction(tx, uidRef)
			if err != nil {
				return err
			}

			deleteUIDMapping = exists && mapping.MemberID == id
		}

		if err := tx.Delete(memberRef); err != nil {
			return fmt.Errorf("delete member %q: %w", id, err)
		}

		if deleteUIDMapping {
			if err := tx.Delete(uidRef); err != nil {
				return fmt.Errorf("delete member UID mapping %q: %w", uid, err)
			}
		}
	...
	if err != nil {
		...
		return fmt.Errorf("delete member transaction: %w", err)
	}
...
func applyMemberPatch(m memdom.Member, patch memdom.MemberPatch, now time.Time) (memdom.Member, error) {
	now = now.UTC()
...
func normalizeMemberForCreate(m memdom.Member, now time.Time) memdom.Member {
	now = now.UTC()
	m = normalizeMemberValues(m)
...
func readMemberSnapshot(doc *firestore.DocumentSnapshot) (memdom.Member, error) {
	if doc == nil {
		return memdom.Member{}, errors.New("member document snapshot is nil")
	}
...
func readMemberUIDSnapshot(doc *firestore.DocumentSnapshot) (memberUIDDocument, error) {
	if doc == nil {
		return memberUIDDocument{}, errors.New("member UID document snapshot is nil")
	}

	var stored memberUIDDocument
	if err := doc.DataTo(&stored); err != nil {
		return memberUIDDocument{}, fmt.Errorf("decode member UID mapping %q: %w", doc.Ref.ID, err)
	}

	if stored.MemberID == "" {
		return memberUIDDocument{}, fmt.Errorf("member UID mapping %q has empty memberId", doc.Ref.ID)
	}
...
func getMemberUIDMappingInTransaction(
	...
	if err != nil {
		...
		return memberUIDDocument{}, false, fmt.Errorf("get member UID mapping %q in transaction: %w", ref.ID, err)
	}
