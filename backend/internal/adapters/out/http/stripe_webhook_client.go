...
func (c *StripeWebhookClient) TriggerPaid(ctx context.Context, invoiceID, billingAddressID string, amount int) error {
	...
	return fmt.Errorf("webhook call failed status=%d body=%s", res.StatusCode, string(body))
}

// trimRight removes all trailing occurrences of cutset from s.
// (strings.TrimRight without importing strings)
func trimRight(s, cutset string) string {
	if cutset == "" {
		return s
	}
	for len(s) >= len(cutset) && s[len(s)-len(cutset):] == cutset {
