param(
  ...
  [string]$ServiceName = "narratives-console",
...
# 1) Go build チェック
# ------------------------------------------------------------
$MainGo = Join-Path $SourceDir "cmd\console\main.go"
if (-not (Test-Path $MainGo)) {
  throw "Go main file not found: $MainGo"
}

Write-Step "go build check (cmd/console)"
...
if ($AutoGenerated) {
  ...
  if ($PodmanAvailable) {
    ...
    $tmp = Join-Path $env:TEMP ("cloudbuild-console-{0}.yaml" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
...
# 6) Cloud Run deploy
# ------------------------------------------------------------
Write-Step "Deploying to Cloud Run (console)"
