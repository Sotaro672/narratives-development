...
#    - TOKEN_ICON_BUCKET / TOKEN_CONTENTS_BUCKET / LIST_BUCKET / AVATAR_ICON_BUCKET を追加
...
$AllowedKeys = @(
  ...
  # ✅ Avatar icon bucket (Signed URL)
  "AVATAR_ICON_BUCKET",

  # ✅ Signed URL signer email (ONLY)
  "GCS_SIGNER_EMAIL",
...
# ✅ Avatar icon bucket defaults（.env が無くても動く保険）
if (-not $envMap.ContainsKey("AVATAR_ICON_BUCKET") -or [string]::IsNullOrWhiteSpace($envMap["AVATAR_ICON_BUCKET"])) {
  $envMap["AVATAR_ICON_BUCKET"] = "narratives-development_avatar_icon"
}

# ✅ Token contents bucket defaults（.env が無くても動く保険）
# NOTE: “登録” = Cloud Run 環境変数に必ず載せる（未設定ならデフォルトを入れる）
if (-not $envMap.ContainsKey("TOKEN_CONTENTS_BUCKET") -or [string]::IsNullOrWhiteSpace($envMap["TOKEN_CONTENTS_BUCKET"])) {
  $envMap["TOKEN_CONTENTS_BUCKET"] = "narratives-development_token_contents"
}

# ✅ SELF_BASE_URL（/mall/me/invoices の checkout に必須）
