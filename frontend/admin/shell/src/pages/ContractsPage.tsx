...
function requireBackendBaseUrl(): string {
  if (!BACKEND_BASE_URL) throw new Error("VITE_BACKEND_BASE_URL is not configured.");
...
export default function ContractsPage() {
  ...
  const loadCompanies = useCallback(async (isCancelled: () => boolean = () => false): Promise<void> => {
    try {
      if (!isCancelled()) {
        setLoading(true);
        setError(null);
      ...
      if (!isCancelled()) setCompanies(result);
    } catch (cause) {
      if (!isCancelled()) {
        setError(cause instanceof Error ? cause.message : "企業一覧の取得に失敗しました。");
      }
    } finally {
  ...
  const columns = useMemo<TableColumn<Company>[]>(() => [
    ...
    },
    {
      ...
      filter: { getValue: (company) => company.name, placeholder: "企業名で絞り込み" },
      nowrap: true,
    },
    {
      ...
      filter: { getValue: (company) => company.representativeName, placeholder: "代表者名で絞り込み" },
      nowrap: true,
    },
    {
      key: "isActive",
