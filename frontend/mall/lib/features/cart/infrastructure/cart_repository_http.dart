...
class CartRepositoryHttp {
  ...
  Future<CartDTO> fetchCart({required String avatarId}) async {
    ...
    final uri = _uri('/mall/cart', qp: {'avatarId': aid});
  ...
  Future<CartDTO> addItem({
    ...
    final uri = _uri('/mall/cart/items');
    ...
    final res = await _sendAuthed('POST', uri, body: jsonEncode(bodyMap));
    return _decodeCart(res);
  }
  ...
  Future<CartDTO> setItemQty({
    ...
    final res = await _sendAuthed('PUT', uri, body: jsonEncode(bodyMap));
    return _decodeCart(res);
  }
  ...
  Future<CartDTO> removeItem({
    ...
    final res = await _sendAuthed('DELETE', uri, body: jsonEncode(bodyMap));
    return _decodeCart(res);
  }
  ...
  CartDTO _decodeCart(http.Response res) {
    if (res.statusCode >= 200 && res.statusCode < 300) {
      ...
      final data = map['data'];
      if (data is Map) {
        return CartDTO.fromJson(data.cast<String, dynamic>());
      }
