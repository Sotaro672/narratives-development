// frontend/mall/src/features/avatar/components/AvatarIconCropper.tsx
...
export type AvatarIconCropperProps = {
  src: string;
  position: AvatarCropPosition;
  scale: number;
  onPositionChange: (position: AvatarCropPosition) => void;
...
type DragState = {
  ...
  startClientX: number;
  startClientY: number;
  startPosition: AvatarCropPosition;
};
...
function positionsEqual(
  left: AvatarCropPosition,
  right: AvatarCropPosition,
): boolean {
  return (
...
export default function AvatarIconCropper({
  src,
  position,
  scale,
  onPositionChange,
  ...
  alt = "アバターアイコンの切り抜きプレビュー",
  disabled = false,
  minScale = DEFAULT_MIN_SCALE,
  maxScale = DEFAULT_MAX_SCALE,
}: AvatarIconCropperProps) {
  ...
  const moveTo = useCallback(
    (nextPosition: AvatarCropPosition) => {
  ...
  return (
    <div className="avatar-icon-cropper">
      <div
        ref={viewportRef}
        className={[
          "avatar-icon-cropper__viewport",
          dragging
            ? "avatar-icon-cropper__viewport--dragging"
            : "",
          disabled
            ? "avatar-icon-cropper__viewport--disabled"
            : "",
        ]
          .filter(Boolean)
          .join(" ")}
      ...
      >
        <img
          src={src}
          alt={alt}
          className="avatar-icon-cropper__image"
          style={imageStyle}
          draggable={false}
        ...
        <div
          className="avatar-icon-cropper__frame"
          aria-hidden="true"
        />
      </div>

      <p className="avatar-icon-cropper__help">
        画像をドラッグして表示位置を調整してください。
      </p>

      <label className="avatar-icon-cropper__zoom">
        <span className="avatar-icon-cropper__zoom-label">
          拡大
        </span>
        ...
        <input
          ...
          disabled={disabled}
          className="avatar-icon-cropper__zoom-input"
          aria-label="画像の拡大率"
          onChange={(event) => {
