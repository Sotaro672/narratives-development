...
export type UseInquiryReplyResult = {
  ...
  onChangeReplyImages: (files: File[]) => void;
...
export function useInquiryReply({
  ...
  const [replyImages, setReplyImages] = useState<ReplyUploadImage[]>([]);
  const [replySubmitting, setReplySubmitting] = useState(false);
  const [replyErrorMessage, setReplyErrorMessage] = useState<string | null>(null);

  const replyImagePreviewUrlsRef = useRef<Set<string>>(new Set());
  ...
  const revokeReplyImagePreviewUrl = useCallback(
  ...
  const clearReplyImages = useCallback((): void => {
    setReplyImages((currentImages: ReplyUploadImage[]) => {
  ...
  const resetReplyForm = useCallback((): void => {
    setReplyContent("");
    setReplyErrorMessage(null);
    clearReplyImages();
  }, [clearReplyImages]);

  const onOpenReplyModal = useCallback((): void => {
    setReplyErrorMessage(null);
    setReplyModalOpen(true);
  }, []);

  const onCloseReplyModal = useCallback((): void => {
    if (replySubmitting) {
      return;
    }
    ...
    resetReplyForm();
  }, [replySubmitting, resetReplyForm]);
  ...
  const onChangeReplyImages = useCallback((files: File[]): void => {
    if (files.length === 0) {
      return;
    }
    ...
    setReplyImages((currentImages: ReplyUploadImage[]) => {
      ...
      const nextImages = acceptedFiles.map(
        (file: File): ReplyUploadImage => {
    ...
    (id: string): void => {
      setReplyImages((currentImages: ReplyUploadImage[]) => {
        const target = currentImages.find(
          (image: ReplyUploadImage) => image.id === id,
        );
  ...
  const onSubmitReply = useCallback(
    async (): Promise<void> => {
      ...
      if (replyImages.length > 0 && !memberId) {
        setReplyErrorMessage(
        ...
        const uploadedImages =
          replyImages.length > 0
            ? await uploadInquiryReplyImagesToStorage({
                inquiryId,
                memberId,
                files: replyImages.map(
                  (image: ReplyUploadImage) => image.file,
                ),
              })
            : [];

        await replyInquiryHTTP(inquiryId, {
