...
export type UseListDetailResult = {
  ...
  onChangeStatus: (next: ListStatus) => void;
...
export function useListDetail(): UseListDetailResult {
  ...
  const onBack = React.useCallback(() => {
    if (progress.isBlockingNavigation || deleting) return;
  ...
  const onCloseProgress = React.useCallback(() => {
    ...
    setProgress(createInitialListProgress());
  }, [progress.isBlockingNavigation]);

  const onEdit = React.useCallback(() => {
    if (deleting || progress.isBlockingNavigation) return;
  ...
  const onSave = React.useCallback(
    async (
      ...
      const effectiveDraftAssigneeId =
        draftAssigneeId || assigneeId;

      if (!effectiveDraftAssigneeId) {
      ...
      try {
        const saveInput = buildListDetailSaveInput({
          ...
          draftDescription,
          draftStatus,
          draftAssigneeId: effectiveDraftAssigneeId,
        ...
        const message =
          errorMessageFromUnknown(caughtError);
