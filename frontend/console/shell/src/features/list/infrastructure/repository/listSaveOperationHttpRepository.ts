// frontend/console/shell/src/features/list/infrastructure/repository/listSaveOperationHttpRepository.ts
...
function normalizeStartListSaveOperationRequest(
  ...
  const listId = requireNonEmptyString(
    input.listId,
    "invalid_list_id",
  );
  ...
  const newImages = Array.isArray(input.newImages)
    ? input.newImages.map((image, index) => {
        ...
        if (!Number.isInteger(displayOrder) || displayOrder < 0) {
          throw new Error(
            `invalid_new_image_display_order_${index}`,
          );
        }
