...
import { Eye, Link2, Upload, X } from "lucide-react";
...
export default function TokenBlueprintCard({
  ...
  // ★ minted:true の場合は、トークン名・シンボル・ブランドを編集不可
      ...
      <CardContent>
            ...
            <div className="token-blueprint-card__field-col">
              ...
              <Input
                ...
                onChange={(e) =>
                  vm.isEditMode &&
                  !isIdentityLocked &&
                  handlers.onChangeName?.(e.target.value)
                }
                readOnly={!vm.isEditMode || isIdentityLocked}
                className={`token-blueprint-card__readonly-input ${
                  !vm.isEditMode || isIdentityLocked ? "readonly" : ""
                }`}
              />
            </div>
                      ...
                      <div className="token-blueprint-card__popover-list">
                        {vm.brandOptions.map((b) => (
                          <button
                            ...
                            onClick={() =>
                              handlers.onChangeBrand?.(b.id, b.name)
                            }
                          >
                            {b.name}
                          </button>
        ...
        <div className="token-blueprint-card__description">
          ...
          <textarea
            ...
            onChange={(e) =>
              vm.isEditMode &&
              handlers.onChangeDescription?.(e.target.value)
            }
