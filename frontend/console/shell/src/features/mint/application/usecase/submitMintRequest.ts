...
export type SubmitMintRequestResult =
  | {
      ok: true;
      queuedResponse: MintQueuedResponse;
    }
  | {
      ok: false;
      reason: "validation" | "empty-response";
      message: string;
    };
...
export async function submitMintRequest(
  ...
  const queuedResponse = await repository.postMintRequest(
    validation.productionId,
    validation.tokenBlueprintId,
  );

  if (!queuedResponse || queuedResponse.status !== "QUEUED") {
    return {
      ok: false,
      reason: "empty-response",
      message: "ミント申請の受付結果を取得できませんでした。",
    };
  }
