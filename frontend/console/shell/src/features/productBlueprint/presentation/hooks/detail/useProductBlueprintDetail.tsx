// frontend/console/productBlueprint/presentation/hooks/detail/useProductBlueprintDetail.tsx
...
function normalizeCategoryFields(
  ...
  for (const [key, fieldValue] of Object.entries(value)) {
    ...
    out[key] =
      normalizeCategoryFieldValue(fieldValue);
  }
  ...
  return removeModelOwnedCategoryFields({
    ...
    material:
      args.material.trim() === ""
        ? null
        : args.material,
    weight: Number.isFinite(args.weight)
      ? args.weight
      : 0,
...
export interface UseProductBlueprintDetailResult {
  ...
  onChangeWashTags: (value: string[]) => void;
...
export function useProductBlueprintDetail():
  ...
  const [assignee, setAssignee] =
    React.useState("担当者未設定");

  const [creator, setCreator] =
    React.useState("作成者未設定");
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const brandNameSvc = String(
          (detail as any).brandName ?? "",
        ).trim();

        const assigneeNameFromService =
          (detail as any).assigneeName as
            | string
            | undefined;

        const createdByNameFromService =
          (detail as any).createdByName as
            | string
            | undefined;

        const updatedByNameFromService =
          (detail as any).updatedByName as
            | string
            | undefined;

        const productBlueprintIdResolved =
        ...
        const categoryFieldsFromDetail =
          removeModelOwnedCategoryFields(
            normalizeCategoryFields(
              (detail as any).categoryFields,
            ),
          );

        const fitFromCategoryFields =
          getStringFromFields(
        ...
        setWashTags(
          washTagsFromCategoryFields.length >
            0
  ...
  const onSave =
    React.useCallback(() => {
      ...
      if (isAlcoholCategory) {
        ...
        if (hasInvalidVolume) {
          alert(
            "容量は 0 より大きい値と単位を入力してください。",
          );

          return;
        }

        const hasEmptyAlcoholModelNumber =
          volumes.some((volume) => {
            const label =
            ...
            return !alcoholModelNumbers.some(
              (modelNumber) =>
                `${modelNumber.volume.value}${String(
                  modelNumber.volume.unit ??
                    "",
                ).trim()}` === label &&
                modelNumber.code.trim(),
            );
          });

        if (
          hasEmptyAlcoholModelNumber
        ) {
          alert(
            "容量ごとのモデルナンバーをすべて入力してください。",
          );
