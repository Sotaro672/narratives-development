...
import { rgbIntToHex, coerceRgbInt } from "../../../../shared/util/color";
...
export function buildSizeRows(
  ...
  return sizeLabels.map((label) => {
    ...
    const mergedMeasurements = mergeMeasurementsBySize(sameSizeVars);
    const sizeMeasurements = mapMeasurementsToApparelSizeInput(
      mergedMeasurements,
      categoryCode,
    );
    return {
