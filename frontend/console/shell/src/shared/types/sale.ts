// frontend/shell/src/shared/types/sale.ts
// Generated from frontend/list/src/domain/entity/sale.ts
// Source of truth for Sale / SalePrice shared types.
...
export const SALE_MAX_PRICE = 10_000_000; // 0 で上限無効、backend は 10_000_000
...
export function isValidSale(sale: Sale): boolean {
  ...
  if (sale.discountId !== undefined && sale.discountId !== null) {
  ...
  for (const p of sale.prices) {
    ...
    if (seen.has(key)) return false; // modelNumber の重複禁止
    seen.add(key);
  }
...
export function aggregateSalePrices(prices: SalePrice[]): SalePrice[] {
  ...
  for (const p of prices) {
    ...
    map.set(model, p.price); // 最後の有効値が勝つ
  }
