...
export async function fetchInvitationInfo(
  ...
  console.log("[InvitationApi] Fetching invitation info:", url);
  ...
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] raw response:", text);
...
export async function fetchCompanyNameById(companyId: string): Promise<string> {
  ...
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] Fetching company name:", url);
  ...
  const text = await res.text();
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] company response:", text);
...
export async function fetchBrandNameById(brandId: string): Promise<string> {
  ...
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] Fetching brand name:", url);
  ...
  const text = await res.text();
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] brand response:", text);
...
export async function fetchBrandNamesByIds(
  ...
  const tasks = ids.map(async (id) => {
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn("[InvitationApi] failed to fetch brand name for id:", id, e);
      return id;
    }
  });
...
export async function validateInvitation(
  ...
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] validating invitation:", url);
  ...
  const text = await res.text();
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] validate response:", text);
...
export async function completeInvitationOnBackend(
  ...
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] complete payload:", payload);
  ...
  const text = await res.text();
  // eslint-disable-next-line no-console
  console.log("[InvitationApi] complete response:", text);
