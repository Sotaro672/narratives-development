...
type EditableConditionMediaItem =
  MediaUploaderItem & {
    source: "existing" | "new";
    file?: File;
...
function getTokenIconUrl(
  ...
  const record =
    item as ResaleListingWithModel & {
      imageUrl?: string;
      tokenIcon?: string;
      tokenIconUrl?: string;
      metadata?: {
...
export default function ResaleDetailPage() {
  ...
  const normalizedResaleId =
    normalizeText(resaleId);

  const [
    item,
    setItem,
  ] =
    useState<ResaleListingWithModel | null>(
      null,
    );
  ...
  const revokeNewPreviewUrls =
    useCallback(() => {
      conditionMediaItems.forEach(
        (mediaItem) => {
          if (
            mediaItem.source ===
              "new" &&
