...
function formatModelColor(
  color: ResaleModelColor | null | undefined,
): string {
  if (!color) {
    return "-";
  }
...
function formatModelVolume(
  volume: ResaleModelVolume | null | undefined,
): string {
  if (!volume) {
    return "-";
  }
...
function getConditionOptionLabel(
  value: ResaleConditionValue,
): string {
  return (
...
export default function ResaleDetailPage() {
  ...
  const [activeGalleryIndex, setActiveGalleryIndex] =
    useState(0);
  const [conditionMediaItems, setConditionMediaItems] =
    useState<EditableConditionMediaItem[]>([]);
  const [
  ...
  const selectedConditionLabel =
    getConditionOptionLabel(conditionInput);
  const selectedStatusLabel =
    getStatusOptionLabel(statusInput);

  const priceNumber = Number(
    priceInput.replace(/[^\d]/g, ""),
  );

  const hasValidPrice =
    Number.isFinite(priceNumber) && priceNumber > 0;

  const hasValidEditableStatus =
  ...
  const resetFormFromItem = useCallback(
    (
      ...
      const nextStatus =
        normalizeEditableStatus(nextItem?.status);
      const nextCondition =
        normalizeCondition(nextItem?.condition);
      ...
      setConditionMediaItems(
        sortImages(nextImages).map(
          createEditableImageItem,
        ),
      );
  ...
  const handleConditionMediaCarouselScroll = () => {
    ...
    setConditionMediaCurrentIndex(
      Math.round(carousel.scrollLeft / width),
    );
  };
  ...
  const handleRemoveConditionMedia = (
    ...
    setConditionMediaItems((current) => {
      const removingItem = current.find(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const sortedNextImages =
        sortImages(nextImages);

      if (sortedNextImages.length > 0) {
        await updatePrimaryResaleImage({
          resaleId: normalizedResaleId,
          imageId: sortedNextImages[0].id,
        });
      }

      const [refreshedItem, refreshedImages] =
        await Promise.all([
          getMyResaleListing(normalizedResaleId),
          listMyResaleConditionImages(
            normalizedResaleId,
