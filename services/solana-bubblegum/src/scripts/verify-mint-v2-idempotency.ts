...
} from "../application/mint-payload-hash.js";
...
import {
  firestore,
} from "../infrastructure/firestore/firestore-client.js";
...
const TEST_TREE_ADDRESS =
  "verify-merkle-tree-address";

const TEST_CORE_COLLECTION_ADDRESS =
  "verify-core-collection-address";

const TEST_LEAF_OWNER_ADDRESS =
  "verify-leaf-owner-address";

const TEST_TOKEN_BLUEPRINT_ID =
  "verify-token-blueprint";

const TEST_BRAND_ID =
  "verify-brand";


class FakeMintV2TransactionClient
  ...
  waitForFinalizedCalls =
    0;

  parseMintResultCalls =
    0;

  failNextBuildRetryable =
    false;
...
function createInput(
  ...
  return {
    ...
    tokenBlueprintId:
      TEST_TOKEN_BLUEPRINT_ID,

    brandId:
      TEST_BRAND_ID,

    leafOwnerAddress:
      TEST_LEAF_OWNER_ADDRESS,
    ...
    coreCollection: {
      ...
      metadataUri:
        "https://example.com/verify-collection.json",
    },
    ...
    metadata: {
      ...
      uri:
        "https://example.com/verify-asset.json",
  ...
  const conflictingInput:
    MintV2UsecaseInput = {
      ...
      metadata: {
        ...
        uri:
          "https://example.com/different-metadata.json",
      },
    };
...
async function verifyRetryableBeforeSigning(
  ...
  transaction.failNextBuildRetryable =
    true;
...
async function verifyFatalFailure(
  ...
  transaction.failNextBuildFatal =
    true;
      ...
      JSON.stringify(
        {
          ...
          retryableSignedTransactionRecovery:
            true,

          retryableBeforeSigningRecovery:
            true,
